...
static inline bool hwdb_flow_finished(const struct hwdb_flow *f) {
	struct timeval n;
	u8 idx = (f->first.packets <= K) ? f->first.packets -1 : K;
	do_gettimeofday(&n);
	if (f->last.tv_sec + HWDB_FLOW_DURATION < n.tv_sec) {
		dbg("[%s] flow finished at %ld (now %ld)\n",
		__FUNCTION__, f->last.tv_sec, n.tv_sec);
	}
	return (
		(f->first.packet[idx].flags & FIN)
		||
		(f->last.tv_sec + HWDB_FLOW_DURATION < n.tv_sec)
	);
}
