	...
	(ntohs(f->key.dst_port) == 80 || ntohs(f->key.dst_port) == 8080)) {
		...
		if (f->request.len == 0) { /* first packet */
			...
			if (eoh) {
				...
				dbg("[%s] warning: ignoring request fragment of size %u\n", 
				__FUNCTION__, f->request.len);
				f->request.header = NULL;
				f->request.len = 0;
				f->request.fin = 1;
...
void hwdb_flow_dump(struct hwdb_flow *flow) {
	...
	for(i = 0; i < K+1; i++) {
		if (flow->first.packet[i].bytes > 0)
			dbg("index %d (of %llu) [%ld]: %llu"FLG_FMT"\n",
			i,
			flow->first.packets,
			flow->first.packet[i].stamp.tv_sec,
	...
	dbg("%s:%s:%s\n",
		(flow->clone & HWDB_FLOW_ACC_EXPORTED ? acc_d : _acc_d),
		(flow->clone & HWDB_FLOW_FNP_EXPORTED ? fnp_d : _fnp_d),
		(flow->clone & HWDB_FLOW_REQ_EXPORTED ? req_d : _req_d)
	);
...
int hwdb_flow_export_requests(struct ht_node *node, void *aux, size_t *index,
	...
	if (!(f->request.len > 0 && f->request.fin == 1)) {
		/* if flow has finished, then reset header */
		if (hwdb_flow_finished(f)) {
			dbg("[%s] warning: http len %u fin %u but flow has finished\n",
			__FUNCTION__, f->request.len, f->request.fin);
			hwdb_flow_request_reset(f);
			f->clone |= HWDB_FLOW_REQ_EXPORTED;
		}
	...
	if (idx + sizeof(struct __hwdb_request) > count) {
